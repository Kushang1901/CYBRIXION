...
export default function Card({
  ...
  const variants = {
    light: 'bg-white border-slate-200/80 text-text-light-primary shadow-sm',
  ...
  const hoverStyles = hoverGlow 
    ? (variant === 'light' 
        ? 'hover:-translate-y-1 hover:shadow-md hover:border-brand-blue/30' 
        : 'hover:-translate-y-1 hover:shadow-[0_0_25px_rgba(34,211,238,0.15)] hover:border-brand-cyan/40')
    : onClick 
    ? 'hover:cursor-pointer hover:bg-slate-50 dark:hover:bg-brand-elevated' 
    : '';
